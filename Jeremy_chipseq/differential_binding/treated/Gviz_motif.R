...
# obtaining-sequences-in-ranges -------------------------------------------

smad2_sequences <- getSeq(x = BSgenome.Hsapiens.UCSC.hg38, ambi_ranges_ls2_vs_ns3$Smad2)

vs2 <- df_ambi_ranges_ls2_vs_ns3_smad2 %>% dplyr::select(GENENAME) %>% unlist()

vs2 <- vs2 %>% str_replace_all(string = ., pattern = " ", replacement = "-")

unique_sequence_id_s2 <- vector(mode = "character", length = 0)

for(i in seq_along(vs2)){
  unique_sequence_id_s2 <- append(unique_sequence_id_s2, paste(vs2[i], sum(vs2[1:i] == vs2[i]), sep = "-"))
}


smad3_sequences <- getSeq(x = BSgenome.Hsapiens.UCSC.hg38, ambi_ranges_ls2_vs_ns3$Smad3)

vs3 <- df_ambi_ranges_ls2_vs_ns3_smad3 %>% dplyr::select(GENENAME) %>% unlist()

vs3 <- vs3 %>% str_replace_all(string = ., pattern = " ", replacement = "-")

unique_sequence_id_s3 <- vector(mode = "character", length = 0)

for(i in seq_along(vs3)){
  unique_sequence_id_s3 <- append(unique_sequence_id_s3, paste(vs3[i], sum(vs3[1:i] == vs3[i]), sep = "-"))
}




names(smad2_sequences) <- unique_sequence_id_s2

names(smad3_sequences) <- unique_sequence_id_s3

writeXStringSet(x = smad2_sequences, filepath = "smad2_sequences.fasta", format = "fasta")

writeXStringSet(x = smad3_sequences, filepath = "smad3_sequences.fasta", format = "fasta")
