...
Differential peaks were called on ChIP-seq data for Smad2- and Smad3-treated cells. Peaks were called using MACS2 to generate bedgraph files, which were then used to call differential peaks using the MACS2 `bdgdiff` module. 

ChIPseq against Smad3 was carried out using two different antibodies:
1. One targeting the GFP portion of the Smad3 fusion protein.
2. One targeting the 
...
### Looking at the percentage of promoter-proximal peaks
...
To further investigate "ambigouous" genes separately, the following variables from the workspace were saved to an RData object for loading during the investigation.
...
ego_prom_genes_ls2_vs_ns3_smad3enriched@result %>%
  ...
  dplyr::select(Description, p.adjust, GeneRatio, BgRatio, geneID)
```
Similar to the LAP-Smad2 versus LAP-Smad3 comparison, GO terms related to regulation of GTPase activity were once again present among the most enriched. However, terms related to cell morphogenesis, migration, cellular adhesion and cytoskeletal organisation and junction regulation also featured in the list of most-enriched GO terms. These GO terms were significantly different from those found for the LAP-Smad2 versus LAP-Smad3 comparison.
