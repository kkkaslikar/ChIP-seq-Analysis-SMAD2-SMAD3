...
**Note**: Since the code for the GO Term analysis takes a long time to process, the relevant code chunks carrying out the analysis in the Rmarkdown source code have been set to "eval = FALSE".The objects generated from the running of that code have been saved separately to the file "differential_binding_go_term.RData". To save time while generating the knitr output for this Rmarkdown document, they will be loaded directly from the aforementioned file in the following code chunk. However, the original source code chunks generating the objects themselves are still displayed. Should the need arise to generate the knitr output without resorting to loading data from "differential_binding_go_term.RData", please set "eval=FALSE" for the following code chunk and "eval=TRUE" for the GO Term analysis code chunks.
